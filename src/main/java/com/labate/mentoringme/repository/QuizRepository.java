...
import com.labate.mentoringme.projection.ViewQuizTakingHistoryListProjection;

@Repository
public interface QuizRepository extends JpaRepository<Quiz, Long> {

	@Query(value = " SELECT DISTINCT(A.id), title, description, number_of_question, type, B.category_id, A.created "
			...
			,countQuery = "SELECT count(distinct(A.id)) "
					...
					+ "	AND (:#{#cond.userId} IS NULL OR A.created_by = :#{#cond.userId}) "
					, nativeQuery = true)
	Page<QuizProjection> findAllByConditions(@Param("cond") FindQuizRequest request, Pageable pageable);
...
//	@Query(value="SELECT A.title, A.number_of_question, B.score, B.created FROM quizzes A join quiz_results B on A.id = B.quiz_id WHERE B.user_id = :userId", nativeQuery=true)
//	List<ViewQuizTakingHistoryListProjection> getQuizTakingHistory(@Param("useId") Long userId);
}
